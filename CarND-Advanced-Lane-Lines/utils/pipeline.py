...
class Pipeline:
    ...
    def set_values(self, l, M, Minv, calibration_matrix, distortion):
        self.line = l
        self.M = M
        self.Minv = Minv
    ...
    def apply_pipeline(self, img):
        (line, M, Minv, calibration_matrix, distortion) = (self.line, self.M, self.Minv, 
            self.calibration_matrix, self.distortion)
        if (line is None or M is None or Minv is None or calibration_matrix is None or distortion is None):
            raise NotImplementedError
            
        img_size = (img.shape[1], img.shape[0])
        width, height = img_size
        img = calibration.undistort(np.copy(img), calibration_matrix, distortion)
